package iad1tya.echo.music.goatify.updater.downloadmanager
...
class UpdateDownloadWorker(private val context: Context, workerParams: WorkerParameters) :
    ...
    override suspend fun doWork(): Result = withContext(Dispatchers.IO) {
        ...
        try {
            ...
            val downloadFile = if (isZip) File(downloadDir, "echo_temp.zip") else File(downloadDir, "goatify.apk")
            ...
            val finalFile = if (isZip) {
                val targetApkFile = File(downloadDir, "goatify.apk")
                var extracted = false
                try {
